...
DisplayManager::DisplayManager(int iWindowWidth, int iWindowHeight)
    : m_iScreenWidth(iWindowWidth), m_iScreenHeight(iWindowHeight), m_pWindow(nullptr), m_pRenderer(nullptr) {}

DisplayManager::~DisplayManager() {
	if (m_pTexture != nullptr) SDL_DestroyTexture(m_pTexture);
	if (m_pRGBBuffer != nullptr) SDL_FreeSurface(m_pRGBBuffer);
	if (m_pScreenBuffer != nullptr) SDL_FreeSurface(m_pScreenBuffer);
	if (m_pRenderer != nullptr) SDL_DestroyRenderer(m_pRenderer);
	if (m_pWindow != nullptr) SDL_DestroyWindow(m_pWindow);
...
void DisplayManager::addColorPalette(WADPalette &palette) { m_ColorPallettes.push_back(palette); }

void DisplayManager::initFrame() { SDL_FillRect(m_pScreenBuffer, NULL, 0); }
...
uint8_t *DisplayManager::getScreenBuffer() {
	return static_cast<uint8_t *>(m_pScreenBuffer->pixels);
}
